    ...
    <div class="card-body">
        <form action="{{ route("admin.services.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name', isset($services) ? $services->name : '') }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group {{ $errors->has('color') ? 'has-error' : '' }}">
                ...
                <textarea id="description" name="description" class="form-control ">{{ old('description', isset($services) ? $services->description : '') }}</textarea>
            ...
            <div class="form-group {{ $errors->has('service_type') ? 'has-error' : '' }}">
                ...
                <input type="text" id="service_type" name="service_type" class="form-control" value="{{ old('service_type', isset($services) ? $services->service_type : '') }}" required>
            ...
            <div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
                ...
                <select name="category_id" id="category_id" class="form-control select2" required>
                    ...
                    @foreach($categories as $id => $category)
                        <option value="{{ $category->id }}" {{ (old('category_id') == $category->id) ? 'selected' : '' }}>{{ $category->name .' - '. $category->id }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('contact_info') ? 'has-error' : '' }}">
                ...
                <textarea id="contact_info" name="contact_info" class="form-control ">{{ old('description', isset($services) ? $services->contact_info : '') }}</textarea>
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                <select name="status" id="status" class="form-control select2" required>
                    ...
                    <option value="1" {{ isset($services->status) && (old('category_id') == $services->status) ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ isset($services->status) && (old('category_id') == $services->status) ? 'selected' : '' }}>In Active</option>
                </select>
